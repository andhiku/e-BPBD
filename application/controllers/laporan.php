...
//        $this->load->library(array('session', 'upload', 'encrypt', 'Excel/PHPExcel', 'PHPExcel/IOFactory'));
    ...
    public function index() {
        ...
        $config['base_url'] = site_url("laporan?data=bencana");
        $config['per_page'] = 20;
        $config['uri_segment'] = 4;
        $config['page_query_string'] = TRUE;
        $config['query_string_segment'] = 'page';
        $config['total_rows'] = $this->db->count_all('tb_bencana');
        $this->pagination->initialize($config);

        $data = array(
            'title' => 'Laporan Data Bencana' . DEFAULT_TITLE,
            'data_bencana' => $this->db->get('tb_bencana', $config['per_page'], $page)->result()
        );
        $this->template->view('v_laporan', $data);
    }

    public function get($ID = 0) {
        $data = $this->db->get_where('tb_bencana', array('id_bencana' => $ID));
        $output = array(
    ...
    public function set_bencana() {
        $id = $this->input->get('id');
        switch ($this->input->get('method')) :
            case 'delete':
                $this->db->delete('tb_bencana', array('id_bencana' => $id));
...
//                    'foto' => $this->input->post('foto'),
//                );
//                $this->db->update('tb_foto', $data_foto, array('id_bencana' => $id));
//                break;
//            case 'delete':
